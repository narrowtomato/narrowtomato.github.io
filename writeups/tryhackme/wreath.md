...
ip-10-200-196-1.eu-west-1.compute.internal (10.200.196.1) at 02:b9:cf:ab:2e:fb [ether] on eth0
...
Ping sweep of local subnet (ran this multiple times to ensure the arp table was filled)

```
[root@prod-serv ~]# for i in {1..255}; do (ping -c 1 10.200.196.${i} | grep "bytes from" &); done
64 bytes from 10.200.196.1: icmp_seq=1 ttl=255 time=0.417 ms
64 bytes from 10.200.196.200: icmp_seq=1 ttl=64 time=0.051 ms
64 bytes from 10.200.196.250: icmp_seq=1 ttl=64 time=0.366 ms
```

Port scan on 10.200.196.250

```
[root@prod-serv ~]# for i in {1..65535}; do (echo > /dev/tcp/10.200.196.250/$i) >/dev/null 2>&1 && echo $i is open; done
22 is open
1337 is open
```
